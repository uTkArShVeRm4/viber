...
fn fs_main(@builtin(position) fragCoord: vec4<f32>) -> @location(0) vec4<f32> {
    ...
    var power_factor: f32;
    if freq_ratio < 0.25 {
        power_factor = 3.0; // Bass: Strong power scaling
    } else if freq_ratio < 0.75 {
        power_factor = 4.0; // Mid: Maximum power scaling for drama
    } else {
        power_factor = 2.5; // Treble: Moderate power scaling
    }
